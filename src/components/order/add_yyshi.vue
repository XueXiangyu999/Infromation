  ...
  methods: {
    submit() {
      ...
      if (this.$route.query.text == null) {
        this.$http
          .post(
            this.$store.state.url + "appointment/manager/lab",
            JSON.stringify({
              equip_name: input[2].value,
              equip_num: input[3].value,
              // id: this.sid,
              // // id: 0,
              name: input[0].value,
              place: input[1].value,
              rule: textarea[0].value,
              stu_num: input[4].value
            }),
            this.config
          )
          .then(res => {
