...
export default {
  ...
  methods: {
    submit() {
      ...
      formdata.append('ceid',this.$route.query.text.ceid)
      ...
      this.$http
        .post(this.$store.state.url +"/evaluation/updateone", formdata, {
          headers: {
  ...
  mounted() {
    ...
    let input = $(".addTable input");
    input[0].value = object.cefaculty;
    input[3].value = object.ceclass;
    input[4].value = object.ceresult;
    this.valueJYS = object.celab;
    this.valueXQ = object.ceterm;
    this.valueLS = object.ceinstructor;
